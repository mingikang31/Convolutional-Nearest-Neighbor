'''CIFAR100 2D data for training 2D CNN Models'''
...
class CIFAR100: 
   def __init__(self, batch_size=64):
      ...
      self.train_data = datasets.CIFAR100(root='./data', train=True, download=True, transform=transform)
      self.test_data = datasets.CIFAR100(root='./data', train=False, download=True, transform=transform)
...
class NoisyCIFAR100(datasets.CIFAR100):
   def __init__(self, 
        ...
        super(NoisyCIFAR100, self).__init__(root, train=train, transform=transform, target_transform=target_transform, download=download)
        self.noise_std = noise_std

   def __getitem__(self, index):
        img, target = super(NoisyCIFAR100, self).__getitem__(index)
        
        # Add Gaussian noise to the image
        noisy_img = img + self.noise_std * torch.randn_like(img)
        return noisy_img, img, target

class CIFAR100_denoise:
    def __init__(self, 
                 batch_size=64, 
                 noise_std=0.3):
        ...
        self.train_data = NoisyCIFAR100(root='./data', train=True, download=True, transform=transform, noise_std=noise_std)
        self.test_data = NoisyCIFAR100(root='./data', train=False, download=True, transform=transform, noise_std=noise_std)
        self.train_loader = DataLoader(dataset=self.train_data, batch_size=self.batch_size, shuffle=True)
        self.test_loader = DataLoader(dataset=self.test_data, batch_size=self.batch_size, shuffle=False)
