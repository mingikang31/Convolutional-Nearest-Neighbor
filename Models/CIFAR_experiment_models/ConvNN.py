...
'''Model Modules'''
### Models ### 
class ConvNN_K_All(nn.Module):
    def __init__(self, in_ch=3, mid_ch=16, num_layers=2, K=9, num_classes=100, device="mps"):
        super(ConvNN_K_All, self).__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(Conv2d_NN(in_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples="all"))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
            else: 
                layers.append(Conv2d_NN(mid_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples="all"))                
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
        ...
        self.device = device
        self.to(self.device)
        self.name = "ConvNN_K_All" # Renamed for clarity
...
class ConvNN_K_N(nn.Module):
    def __init__(self, in_ch=3, mid_ch=16, num_layers=2, K=9, N=64, num_classes=100, device="mps"):
        super(ConvNN_K_N, self).__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(Conv2d_NN(in_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
            else: 
                layers.append(Conv2d_NN(mid_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
        ...
        self.device = device
        self.to(self.device)
        self.name = "ConvNN_K_N" # Renamed for clarity
...
class ConvNN_Spatial_K_N(nn.Module):
    def __init__(self, in_ch=3, mid_ch=16, num_layers=2, K=9, num_classes=100, N=8, device="mps"):
        super(ConvNN_Spatial_K_N, self).__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(Conv2d_NN_spatial(in_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
            else: 
                layers.append(Conv2d_NN_spatial(mid_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
        ...
        self.device = device
        self.to(self.device)
        self.name = "ConvNN_Spatial_K_N" # Renamed for clarity
...
class ConvNN_Attn_K_All(nn.Module):
    def __init__(self, in_ch=3, mid_ch=16, num_layers=2, K=9, image_size=(32, 32), num_classes=100,  device="mps"):
        super(ConvNN_Attn_K_All, self).__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(Conv2d_NN_Attn(in_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples="all", image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
            else: 
                layers.append(Conv2d_NN_Attn(mid_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples="all", image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
        ...
        self.device = device
        self.to(self.device)
        self.name = "ConvNN_Attn_K_All" # Renamed for clarity
...
class ConvNN_Attn_K_N(nn.Module):
    def __init__(self, in_ch=3, mid_ch=16, num_layers=2, K=9, N=64, image_size=(32, 32), num_classes=100, device="mps"):
        super(ConvNN_Attn_K_N, self).__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(Conv2d_NN_Attn(in_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N, image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
            else: 
                layers.append(Conv2d_NN_Attn(mid_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N, image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
        ...
        self.device = device
        self.to(self.device)
        self.name = "ConvNN_Attn_K_N" # Renamed for clarity
...
class ConvNN_Attn_Spatial_K_N(nn.Module):
    def __init__(self, in_ch=3, mid_ch=16, num_layers=2, K=9, N=8, image_size=(32, 32), num_classes=100, device="mps"):
        super(ConvNN_Attn_Spatial_K_N, self).__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(Conv2d_NN_Attn_spatial(in_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N, image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
            else: 
                layers.append(Conv2d_NN_Attn_spatial(mid_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N, image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
        ...
        self.device = device
        self.to(self.device)
        self.name = "ConvNN_Attn_Spatial_K_N" # Renamed for clarity
...
class ConvNN_Attn_V_K_All(nn.Module):
    def __init__(self, in_ch=3, mid_ch=16, num_layers=2, K=9, image_size=(32, 32), num_classes=100,  device="mps"):
        super(ConvNN_Attn_V_K_All, self).__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(Conv2d_NN_Attn_V(in_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples="all", image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
            else: 
                layers.append(Conv2d_NN_Attn_V(mid_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples="all", image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
        ...
        self.device = device
        self.to(self.device)
        self.name = "ConvNN_Attn_V_K_All" # Renamed for clarity
...
class ConvNN_Attn_V_K_N(nn.Module):
    def __init__(self, in_ch=3, mid_ch=16, num_layers=2, K=9, N=64, image_size=(32, 32), num_classes=100, device="mps"):
        super(ConvNN_Attn_V_K_N, self).__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(Conv2d_NN_Attn_V(in_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N, image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
            else: 
                layers.append(Conv2d_NN_Attn_V(mid_ch, mid_ch, K=K, stride=K, shuffle_pattern="BA", shuffle_scale=2, samples=N, image_size=image_size))
                # layers.append(nn.BatchNorm2d(mid_ch)) 
                layers.append(nn.ReLU())
        ...
        self.device = device
        self.to(self.device)
        self.name = "ConvNN_Attn_V_K_N" # Renamed for clarity
...
'''Register Models'''
@register_model
def convnn_k_all_100(pretrained=False, **kwargs):
    """ConvNN K All Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_K_All(in_ch=3, mid_ch=16, num_layers=2, K=9, num_classes=100, device='mps')
...
@register_model
def convnn_k_all_10(pretrained=False, **kwargs):
    """ConvNN K All Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_K_All(in_ch=3, mid_ch=16, num_layers=2, K=9, num_classes=10, device='mps')
...
@register_model
def convnn_k_n_100(pretrained=False, **kwargs):
    """ConvNN K N Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=64, num_classes=100, device='mps')
...
@register_model
def convnn_k_n_10(pretrained=False, **kwargs):
    """ConvNN K N Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=64, num_classes=10, device='mps')
...
@register_model
def convnn_spatial_k_n_100(pretrained=False, **kwargs):
    """ConvNN Spatial K N Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Spatial_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=8, num_classes=100, device='mps')
...
@register_model
def convnn_spatial_k_n_10(pretrained=False, **kwargs):
    """ConvNN Spatial K N Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Spatial_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=8, num_classes=10, device='mps')
...
@register_model
def convnn_attention_k_all_100(pretrained=False, **kwargs):
    """ConvNN Attention K All Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_K_All(in_ch=3, mid_ch=16, num_layers=2, K=9, image_size=(32, 32), num_classes=100, device='mps')
...
def convnn_attention_k_all_10(pretrained=False, **kwargs):
    """ConvNN Attention K All Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_K_All(in_ch=3, mid_ch=16, num_layers=2, K=9, image_size=(32, 32), num_classes=10, device='mps')
...
@register_model
def convnn_attention_k_n_100(pretrained=False, **kwargs):
    """ConvNN Attention K N samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=64, image_size=(32, 32), num_classes=100, device='mps')
...
@register_model
def convnn_attention_k_n_10(pretrained=False, **kwargs):
    """ConvNN Attention K N samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=64, image_size=(32, 32), num_classes=10, device='mps')
...
@register_model
def convnn_attention_spatial_k_n_100(pretrained=False, **kwargs):
    """ConvNN Attention K N samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_Spatial_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=8, image_size=(32, 32), num_classes=100, device='mps')
...
@register_model
def convnn_attention_spatial_k_n_10(pretrained=False, **kwargs):
    """ConvNN Attention K N samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_Spatial_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=8, image_size=(32, 32), num_classes=10, device='mps')
...
@register_model
def convnn_attention_v_k_all_100(pretrained=False, **kwargs):
    """ConvNN Attention V K All Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_V_K_All(in_ch=3, mid_ch=16, num_layers=2, K=9, image_size=(32, 32), num_classes=100, device='mps')
...
@register_model
def convnn_attention_v_k_all_10(pretrained=False, **kwargs):
    """ConvNN Attention V K All Samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_V_K_All(in_ch=3, mid_ch=16, num_layers=2, K=9, image_size=(32, 32), num_classes=10, device='mps')
...
@register_model
def convnn_attention_v_k_n_100(pretrained=False, **kwargs):
    """ConvNN Attention K N samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_V_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=64, image_size=(32, 32), num_classes=100, device='mps')
...
@register_model
def convnn_attention_v_k_n_10(pretrained=False, **kwargs):
    """ConvNN Attention K N samples"""
    # Ensure this function now uses the updated CNN class
    model = ConvNN_Attn_V_K_N(in_ch=3, mid_ch=16, num_layers=2, K=9, N=64, image_size=(32, 32), num_classes=10, device='mps')
...
if __name__ == "__main__":
    ...
    models = [
        ...
        convnn_k_n_100(),
        convnn_k_n_10(),
        convnn_spatial_k_n_100(),
        convnn_spatial_k_n_10(),
        convnn_attention_k_all_100(),
        convnn_attention_k_all_10(),
        convnn_attention_k_n_100(), 
        convnn_attention_k_n_10(),
        convnn_attention_spatial_k_n_100(), 
        convnn_attention_spatial_k_n_10(),
        convnn_attention_v_k_all_100(),
        convnn_attention_v_k_all_10(),
        convnn_attention_v_k_n_100(),
        convnn_attention_v_k_n_10()
